...
# Init. Training
model.train_init(hdc_model, inp_enc=train_enc, target=ds_train[1], binary=binary)
test_acc = model.test(hdc_model, inp_enc=test_enc, target=ds_test[1], binary=binary)
...
for i in tqdm.tqdm(range(train_epochs)):
    ...
    if (i + 1) % val_epochs == 0:
        test_acc = model.test(
            hdc_model, inp_enc=test_enc, target=ds_test[1], binary=binary
        )
